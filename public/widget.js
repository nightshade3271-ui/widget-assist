...
console.log('WidgetAssist: SCRIPT START');

const script = document.currentScript || document.querySelector('script[data-widget-id]');
const widgetId = script?.getAttribute('data-widget-id');
const API_BASE = script ? new URL(script.src).origin : '';

if (!widgetId) {
  console.error('WidgetAssist: data-widget-id is required');
} else {
  ...
  const styles = `
    .wm-widget-container {
      ...
      font-family: 'ABeeZee', 'Inter', system-ui, sans-serif;
    }
    .wm-launcher {
      width: 60px;
      height: 60px;
    ...
    .wm-header h3 { margin: 0; font-size: 13px; font-weight: 600; white-space: nowrap; }
    .wm-header .wm-header-sep { opacity: 0.5; font-size: 11px; }
    .wm-header p { margin: 0; font-size: 11px; opacity: 0.7; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .wm-banner {
      width: 100%;
      max-height: 100px;
      overflow: hidden;
  ...
  // Initialize
  console.log('WidgetAssist: Initializing...', { widgetId, API_BASE });

  fetch(`${API_BASE}/api/widget/${widgetId}`, { mode: 'cors', credentials: 'omit' })
    .then(res => {
      console.log('WidgetAssist: Fetch response', res.status);
      if (!res.ok) {
    ...
    .then(config => {
      console.log('WidgetAssist: Config loaded', config);
      initWidget(config);
    })
    .catch(err => {
      console.error('WidgetAssist: Failed to load config', err);
    });
  ...
  function initWidget(config) {
    ...
    const getBannerHtml = () => {
      if (config.bannerUrl) {
        return `<div class="wm-banner"><img src="${config.bannerUrl}" alt=""></div>`;
      }
      return '';
    };
